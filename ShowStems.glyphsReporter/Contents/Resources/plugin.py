...
from Cocoa import NSBezierPath, NSColor, NSControlKeyMask, NSCommandKeyMask, NSAlternateKeyMask, NSMakeRect, NSRect
import traceback
# import numpy as np
...
class ShowVerticalStems(ReporterPlugin):
	...
	@objc.python_method
	def BoundsRect(self, rect: NSRect):
		x = rect[0][0]
		y = rect[0][1]
		width = rect[1][0]
		height = rect[1][1]
	...
	@objc.python_method
	def drawLine(self, color, x1, y1, x2, y2, w=1.0):
	...
	@objc.python_method
	def drawBadge(self, x, y, width: float, color):
		height = width
		myRect = NSMakeRect(x - width / 2, y - height / 2, width, height)
	...
	def drawRoundedRectangleForStringAtPosition(self, thisString, center, fontsize, color=(0, .3, .8, .65)):
		...
		rim = scaledSize * 0.3
		panel = NSMakeRect(x - width / 2, y - scaledSize / 2 - rim, width, scaledSize + rim * 2)
		NSColor.colorWithCalibratedRed_green_blue_alpha_(*color).set()
	...
	def verticalStems(self, layer):
		...
		MeasurementPositionMeasurementLine = Glyphs.floatDefaults["MeasurementPositionMeasurementLine"]
		Dimensions = {}
		savedMeasurements = {}
